...
// --- Geocode cache persistente su DB (per evitare reverse-geocode durante export) ---
async function getGeocodeCacheLabelDb(key) {
  try {
    const { rows } = await query(
      "SELECT label FROM geocode_cache WHERE key = $1",
      [key],
    );
    return rows && rows[0] ? rows[0].label : null;
  } catch (err) {
    console.warn("Geocode cache DB read failed:", err?.message || err);
    return null;
  }
}

async function upsertGeocodeCacheLabelDb(key, label) {
  if (!key || !label) return;
  try {
    await query(
      `INSERT INTO geocode_cache (key, label)
       VALUES ($1, $2)
       ON CONFLICT (key)
    ...
    console.warn("Geocode cache DB upsert failed:", err?.message || err);
  }
}

function buildGeocodeKeyFromCoords(lat, lon) {
  return `${Number(lat).toFixed(4)},${Number(lon).toFixed(4)}`;
}

async function geocodeLocalityFromLocationString(rawLocation) {
  const trimmed = typeof rawLocation === "string" ? rawLocation.trim() : "";
  if (!trimmed) return "";
  const coords = extractCoordsFromLocationString(trimmed);

  // Se è già un indirizzo testuale, estrai "località" senza chiamate esterne
  if (!coords) {
    return extractLocalityFromAddressString(trimmed);
  }

  const key = buildGeocodeKeyFromCoords(coords.lat, coords.lon);

  // 1) cache DB
  const cachedDb = await getGeocodeCacheLabelDb(key);
  if (cachedDb) return cachedDb;

  // 2) cache in-memory (già gestita da reverseGeocodeCoordinates)
  const label = await reverseGeocodeCoordinates(coords.lat, coords.lon);
  const value = label || "Località non disponibile";

  // 3) salva in cache DB (persistente)
  await upsertGeocodeCacheLabelDb(key, value);
  return value;
}

// Aggiorna start_locality / end_locality di una entry.
// - prova a risolvere località per start_location/end_location
// - se fallisce, lascia "Località non disponibile" (ma mai vuoto)
async function fillEntryLocalities(entryId, startLocationRaw, endLocationRaw) {
  if (!entryId) return;

  const [startLabel, endLabel] = await Promise.all([
    geocodeLocalityFromLocationString(startLocationRaw),
    geocodeLocalityFromLocationString(endLocationRaw),
  ]);

  await query(
    `UPDATE entries
     SET start_locality = COALESCE($2, start_locality),
         end_locality   = COALESCE($3, end_locality)
     WHERE id = $1`,
    [
      entryId,
      startLabel ? startLabel : "Località non disponibile",
      endLabel ? endLabel : "Località non disponibile",
    ],
  );
}

// Backfill a lotti: riempie località mancanti senza bloccare le request web.
async function backfillGeocodesBatch(limit = 25) {
  const { rows } = await query(
    `SELECT id, start_location, end_location, location
     FROM entries
     WHERE (start_location IS NOT NULL AND start_locality IS NULL)
        OR (end_location   IS NOT NULL AND end_locality   IS NULL)
     ORDER BY id DESC
     LIMIT $1`,
    [limit],
  );
  ...
  for (const r of rows) {
    const startRaw = coalesce(r.start_location, r.location, "");
    const endRaw = coalesce(r.end_location, "");
    try {
      await fillEntryLocalities(r.id, startRaw, endRaw);
    } catch (err) {
      console.warn(
        "Backfill geocodes failed for entry",
        r.id,
        err?.message || err,
      );
    }
...
async function createEntryInDb(entry, req) {
  ...
  // Best-effort: prova a salvare subito le località (2 reverse-geocode max).
  // Se per qualunque motivo è lento, continuiamo in background.
  const startRaw = coalesce(inserted.start_location, inserted.location, "");
  const endRaw = coalesce(inserted.end_location, "");
  const fillPromise = fillEntryLocalities(inserted.id, startRaw, endRaw);
  ...
  // Se non ha finito entro timeout, continua in background.
  fillPromise.catch(() => {});
  return inserted;
}
...
app.post("/api/export/csv", authMiddleware, async (req, res) => {
  try {
    ...
    // Se mancano località, NON facciamo reverse-geocode durante export (rischio H12 su Heroku).
    // Triggeriamo un backfill e chiediamo di riprovare tra poco: così il CSV avrà SEMPRE la località.
    const missingLocalities = rows.filter((r) => {
      const startNeeds =
        coalesce(r.start_location, r.location, "").trim() !== "" &&
        coalesce(r.start_locality, "").trim() === "";
      const endNeeds =
        coalesce(r.end_location, "").trim() !== "" &&
        coalesce(r.end_locality, "").trim() === "";
      return startNeeds || endNeeds;
    });

    if (missingLocalities.length > 0) {
      // backfill in background (batch limitato)
      setImmediate(async () => {
        try {
          // facciamo più batch piccoli, ma senza saturare
          for (let i = 0; i < 4; i++) {
            const done = await backfillGeocodesBatch(25);
            if (!done) break;
          }
        } catch (err) {
          console.warn(
            "Backfill geocodes during export failed:",
            err?.message || err,
          );
        }
      });
      ...
      return res.status(409).json({
        error:
          "Geocodifica località in corso (Geo inizio/fine). Riprova l'export tra 30-60 secondi.",
        missing: missingLocalities.length,
      });
    }
    ...
    const csvColumns = [
      ...
      { header: "Geo inizio", key: "start_locality" },
      { header: "Geo fine", key: "end_locality" },
    ...
    for (const e of rows) {
      const startLocation = coalesce(e.start_locality, "");
      const endLocation = coalesce(e.end_locality, "");
      const lineData = {
        ...
        start_locality: startLocation,
        end_locality: endLocation,
...
// --- Modalità backfill (per Heroku Scheduler) ---
// Esempio: `node app.js --backfill-geocodes`
// Fa piccoli batch e poi termina.
if (process.argv.includes("--backfill-geocodes")) {
  (async () => {
    ...
    console.log(`Backfill geocodes completato. Righe aggiornate: ${total}`);
    process.exit(0);
  })().catch((err) => {
    console.error("Backfill geocodes fallito:", err);
    process.exit(1);
  });
}
